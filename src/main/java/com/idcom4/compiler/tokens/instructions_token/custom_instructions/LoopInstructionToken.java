...
public class LoopInstructionToken extends NestedInstructionToken {

    public static final String name = IdASMCompiler.EKeywords.LOOP.value;

    public static class Parser {

        public static ParsingResult<LoopInstructionToken> TryParse(String sourceCode, int index) throws CompilationException {
        ...
        public static short ComputeAddressShift(CmpValueToken cmpToken) {
            int instructions = 2;
            if (cmpToken.GetOperator().equals(CmpValueToken.ECmpOperators.EQ))
                instructions += 1;

            return (short) (instructions * DirectInstructionToken.ADDRESS_AMOUNT);
        }
    }

    private final String exitLoopHiddenLabel;
    private final String gotoLoopHiddenLabel;
    ...
    private GotoInstructionToken gotoInstructionToken = null;

    private final int addressesAmount;


    public LoopInstructionToken(DirectValueToken param0, CmpValueToken cmpOperator, DirectValueToken param1, List<InstructionToken> subInstructions) throws DuplicateIdentifierException, CustomValueException {
        super(subInstructions);

        // generate loop exit hidden label
        String hiddenLabel;
        do {
            hiddenLabel = UUID.randomUUID().toString();
        } while (Context.INSTANCE.labels.IsAlreadyRegistered(hiddenLabel));
        this.exitLoopHiddenLabel = hiddenLabel;

        // generate loop head label
        do {
            hiddenLabel = UUID.randomUUID().toString();
        } while (Context.INSTANCE.labels.IsAlreadyRegistered(hiddenLabel));
        this.gotoLoopHiddenLabel = hiddenLabel;
        ...
        try {
            Pair<MoveInstructionToken, JmpInstructionToken[]> controlInstructions = this.CreateIfControlInstructions(this.exitLoopHiddenLabel, cmpOperator, param0, param1);
        ...
        this.gotoInstructionToken = this.CreateGotoInstruction(this.gotoLoopHiddenLabel);

        this.addressesAmount = this.ComputeAddressesAmount(subInstructions, moveInstruction, jumpInstructions, gotoInstructionToken);

        // register labels
        Context.INSTANCE.labels.Register(gotoLoopHiddenLabel, (short)(Context.INSTANCE.addressSpace.GetCurrentAddress() - this.addressesAmount));
        Context.INSTANCE.labels.Register(exitLoopHiddenLabel);
    }
    ...
    public byte[] Generate() throws UnknownLabelException, CustomValueException {
        ...
        // goto
        byte[] gotoBytes = this.gotoInstructionToken.Generate();
    ...
    private GotoInstructionToken CreateGotoInstruction(String loopHeadLabel) {
        return new GotoInstructionToken(new LabelValueToken(loopHeadLabel));
    }

    private int ComputeAddressesAmount(List<InstructionToken> subInstructions, MoveInstructionToken moveInstruction, JmpInstructionToken[] jumpInstructions, GotoInstructionToken gotoInstruction) throws CustomValueException {
        ...
        int gotoInstructionAddressAmount = gotoInstruction.GetAmountOfAddresses();

        return subInstructionsAddressAmount + moveInstructionAddressAmount + jumpInstructionsAddressAmount + gotoInstructionAddressAmount;
    }
